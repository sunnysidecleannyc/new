(function(){
  ...
  var s;try{s=localStorage.getItem('nycmaid_sid');}catch(e){}
  s=s||Math.random().toString(36).substr(2,9);
  try{localStorage.setItem('nycmaid_sid',s);}catch(e){}
  ...
  function detectCTA(el){
    ...
    if(a){
      ...
      if(h.indexOf('/book')!==-1) return {action:'book',placement:t,href:h};
    ...
    if(btn){
      ...
      if(da==='book'||dh.indexOf('/book')!==-1) return {action:'book',placement:bt,href:dh};
