...
import { areaPageSchemas, neighborhoodPageSchemas } from '@/lib/seo/schema'
...
const SERVICE_LOCATION_SLUGS = [
  ...
  'tribeca-cleaning-services',
  'williamsburg-cleaning-services',
  'midtown-cleaning-services',
  'east-village-cleaning-services',
]

function getLocationData(slug: string) {
  // Try as area first, then as neighborhood
  const area = getAreaByUrlSlug(slug)
  if (area) return { type: 'area' as const, area }
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (area) {
    ...
    const title = `Professional House Cleaning in ${area.name} — From $49/hr | Sunnyside Clean NYC`
  ...
  if (neighborhood) {
    ...
    const url = `https://www.cleaningservicesunnysideny.com/service-locations/${slug}`
    const title = `Professional Cleaning Services in ${neighborhood.name} — From $49/hr | Sunnyside Clean NYC`
    const description = `Trusted cleaning professionals serving ${neighborhood.name}, ${neighborhoodArea.name}. ${neighborhood.housing_types.slice(0, 2).join(', ')} specialists near ${neighborhood.landmarks[0]}. From $49/hr. (212) 202-8400`
    return {
      title: { absolute: title },
      description,
      alternates: { canonical: url },
...
export default async function ServiceLocationPage({ params }: Props) {
    ...
    return (
      <>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <Breadcrumbs items={[{ name: area.name, href: `/service-locations/${area.urlSlug}` }]} />
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16 items-start">
              ...
              <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {neighborhoods.map(n => (
                  <Link
                    key={n.slug}
                    href={`/service-areas/${n.urlSlug}`}
                    className="group border border-gray-200 rounded-xl p-5 hover:border-[#A8F0DC] hover:shadow-md transition-all bg-white"
                  >
                    <h3 className="font-semibold text-[#1E2A4A] group-hover:text-[#1E2A4A]/70 transition-colors mb-1">{n.name}</h3>
                    <p className="text-gray-400 text-sm">{n.housing_types.slice(0, 2).join(' · ')}</p>
                    <p className="text-[#1E2A4A] text-xs font-medium mt-3 group-hover:underline underline-offset-4">View Services &rarr;</p>
                  </Link>
                ))}
              </div>
            </div>
        ...
        <section className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <p className="font-[family-name:var(--font-bebas)] text-3xl md:text-4xl text-[#1E2A4A] tracking-wide text-center mb-4">Every Cleaning Service Available in {area.name}</p>
            <p className="text-gray-500 text-center max-w-2xl mx-auto mb-12">From weekly cleaning service to deep cleaning, move-in/out, and same-day emergency cleans — all at the same flat hourly rate.</p>
        ...
        <section className="py-20 bg-[#1E2A4A]">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { n: '01', t: 'Book Online or Call', d: `Visit thenycmaid.com/book or call (212) 202-8400 with your ${area.name} address, preferred date, and any special requests.` },
        ...
        <CTABlock title={`Book Your ${area.name} Cleaning Today`} subtitle={`Book online or call — serving every ${area.name} neighborhood at the same flat hourly rate.`} />
      </>
    )
  }
  ...
  if (neighborhood) {
    ...
    return (
      <>
        <JsonLd data={neighborhoodPageSchemas(neighborhood, neighborhoodArea)} />
        ...
        <section className="bg-white border-b border-gray-100 pt-14 md:pt-20 pb-0">
          <div className="max-w-5xl mx-auto px-4 text-center">
            <div className="flex items-center justify-center gap-2 mb-5">
              <Link href={`/service-areas/${neighborhoodArea.urlSlug}`} className="text-xs font-semibold text-[#A8F0DC] tracking-[0.2em] uppercase hover:text-[#1E2A4A] transition-colors">{neighborhoodArea.name}</Link>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
              <a href="https://www.thenycmaid.com/book" className="bg-[#1E2A4A] text-white px-10 py-4 rounded-lg font-bold text-sm tracking-widest uppercase hover:bg-[#1E2A4A]/90 transition-colors">
                Book a Cleaning
              </a>
            ...
            <div className="max-w-5xl mx-auto px-4 py-5">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-blue-200/50 text-sm">5.0 on Google</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <Breadcrumbs items={[
            { name: neighborhoodArea.name, href: `/service-areas/${neighborhoodArea.urlSlug}` },
            { name: neighborhood.name, href: `/service-locations/${neighborhood.urlSlug}` },
          ]} />
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
              ...
              <div className="space-y-6">
                <div className="bg-[#F5FBF8] border border-[#A8F0DC]/30 rounded-2xl p-8">
                  <h3 className="font-[family-name:var(--font-bebas)] text-2xl text-[#1E2A4A] tracking-wide mb-5">{neighborhood.name} at a Glance</h3>
                  <div className="grid grid-cols-2 gap-6">
                    {funFacts.map(fact => (
                      <div key={fact.label}>
                        <p className="font-[family-name:var(--font-bebas)] text-3xl text-[#1E2A4A] tracking-wide">{fact.value}</p>
                ...
                <div className="border border-gray-200 rounded-2xl p-8">
                  <h3 className="font-[family-name:var(--font-bebas)] text-2xl text-[#1E2A4A] tracking-wide mb-5">{neighborhood.name} Is Known For</h3>
        ...
        <section className="py-16 bg-gradient-to-b from-[#1E2A4A] to-[#243352]">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-xs font-semibold text-[#A8F0DC]/60 tracking-[0.25em] uppercase mb-3 text-center">Local Cleaning Expertise</h2>
        ...
        <section className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-xs font-semibold text-gray-400 tracking-[0.25em] uppercase mb-3 text-center">Cleaning Services</h2>
            <p className="font-[family-name:var(--font-bebas)] text-3xl md:text-4xl text-[#1E2A4A] tracking-wide text-center mb-4">Every Cleaning Service Available in {neighborhood.name}</p>
            <p className="text-gray-500 text-center max-w-2xl mx-auto mb-12">From weekly cleaning service to deep cleaning, move-in/out, and same-day emergency cleans — all at the same flat hourly rate.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {SERVICES.map(s => (
                <Link
        ...
        <section className="py-20 bg-[#1E2A4A]">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { n: '01', t: 'Book Online or Call', d: `Visit thenycmaid.com/book or call (212) 202-8400 with your ${neighborhood.name} address, preferred date, and any special requests.` },
                { n: '02', t: 'We Confirm', d: 'We match you with a background-checked, insured cleaner and lock in your appointment — usually within the hour.' },
                { n: '03', t: 'Pay After', d: 'Your cleaner arrives on time, does the work, and you pay only after the cleaning is complete. No deposits ever.' },
              ].map(s => (
                <div key={s.n} className="bg-white/[0.06] backdrop-blur-sm border border-white/10 rounded-2xl p-7 text-center">
        ...
        {neighborhood.nearby.length > 0 && (
          <section className="py-20 bg-white">
            <div className="max-w-7xl mx-auto px-4">
              ...
              <p className="font-[family-name:var(--font-bebas)] text-3xl md:text-4xl text-[#1E2A4A] tracking-wide text-center mb-4">Neighborhoods Near {neighborhood.name}</p>
        ...
        <CTABlock title={`Book Your ${neighborhood.name} Cleaning Today`} subtitle={`Book online or call — same rates, same quality across all of ${neighborhoodArea.name}.`} />
      </>
    )
  }
