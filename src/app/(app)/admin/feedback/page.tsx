...
export default function FeedbackPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <DashboardHeader currentPage="feedback" />

      <main className="p-3 md:p-6">
        <div className="flex justify-between items-start mb-2">
          <div>
            <h2 className="text-2xl font-semibold text-[#1E2A4A]">Feedback</h2>
            <p className="text-gray-500">{totalCount} total · {unreadCount} unread</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-[#A8F0DC]/20 rounded-lg p-4 border border-[#A8F0DC]/30">
            <p className="text-sm text-[#1E2A4A]">Total Feedback</p>
            <p className="text-2xl font-bold text-[#1E2A4A]/70">{totalCount}</p>
          </div>
          <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-100">
            <p className="text-sm text-yellow-600">Unread</p>
            <p className="text-2xl font-bold text-yellow-700">{unreadCount}</p>
          </div>
          <div className="bg-green-50 rounded-lg p-4 border border-green-100 hidden md:block">
            <p className="text-sm text-green-600">Read</p>
            <p className="text-2xl font-bold text-green-700">{totalCount - unreadCount}</p>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-6 border-b">
          <button
            ...
            className={`pb-3 px-1 text-sm font-medium ${filter === 'all' ? 'border-b-2 border-[#1E2A4A] text-[#1E2A4A]' : 'text-gray-500'}`}
          >
            All ({totalCount})
          </button>
          <button
            onClick={() => setFilter('unread')}
            className={`pb-3 px-1 text-sm font-medium ${filter === 'unread' ? 'border-b-2 border-[#1E2A4A] text-[#1E2A4A]' : 'text-gray-500'}`}
          >
            Unread ({unreadCount})
          </button>
          <button
            onClick={() => setFilter('read')}
            className={`pb-3 px-1 text-sm font-medium ${filter === 'read' ? 'border-b-2 border-[#1E2A4A] text-[#1E2A4A]' : 'text-gray-500'}`}
          >
            ...
            {filteredFeedback.map((item) => (
              <div
                ...
                className={`bg-white border rounded-lg p-4 transition hover:shadow-md ${
                  !item.read ? 'border-l-4 border-l-[#A8F0DC] bg-[#A8F0DC]/5' : 'border-gray-200'
                }`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${getSourceColor(item.source)}`}>
                        {getSourceLabel(item.source)}
                      </span>
                      {!item.read && (
                        <span className="px-2 py-0.5 bg-[#A8F0DC]/30 text-[#1E2A4A] rounded text-xs font-medium">
                          New
                        </span>
                      )}
                      ...
                      <span className="text-xs text-gray-400 ml-auto whitespace-nowrap" title={formatDate(item.created_at)}>
                    ...
                    {/* Title if present */}
                    {item.title && (
                      <h4 className="font-medium text-[#1E2A4A] mb-1">{item.title}</h4>
                    )}
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    {!item.read && (
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#1E2A4A] text-white rounded text-xs font-medium hover:bg-[#1E2A4A]/90 transition whitespace-nowrap"
                      >
                        Mark Read
                      </button>
                    )}
                    <button
                      ...
                      className="px-3 py-1.5 bg-red-50 text-red-600 rounded text-xs font-medium hover:bg-red-100 transition disabled:opacity-50 whitespace-nowrap"
                    >
