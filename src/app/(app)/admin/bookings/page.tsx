...
function BookingsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (createForm.is_emergency) {
      ...
      // Batch create all bookings in one request
  ...
  const handleCancel = async (scope: 'single' | 'all') => {
    ...
    if (scope === 'all' && editingBooking.recurring_type) {
      ...
      // Cancel first one with email, rest silently
      if (futureBookings.length > 0) {
        await fetch('/api/bookings/' + futureBookings[0].id, { method: 'DELETE' })
        // Cancel the rest in parallel, all with skip_email
        await Promise.all(
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="p-3 md:p-6">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-2xl font-semibold text-[#1E2A4A]">Bookings</h2>
          <div className="flex gap-3">
            <button onClick={() => setShowFilters(!showFilters)} className={'px-4 py-2 border rounded-lg font-medium ' + (showFilters || activeFilterCount > 0 ? 'border-[#1E2A4A] bg-[#1E2A4A] text-white' : 'border-gray-300 text-[#1E2A4A]')}>
              Filters {activeFilterCount > 0 && `(${activeFilterCount})`}
            </button>
            <button onClick={openCreate} className="bg-[#1E2A4A] text-white px-4 py-2 rounded-lg font-medium hover:bg-[#1E2A4A]/90">Create</button>
          </div>
        </div>
        <div className="text-sm text-gray-500 mb-4 hidden md:block">
          ...
          Team portal: <a href="https://www.thenycmaid.com/team" target="_blank" className="text-[#1E2A4A] hover:underline ml-1">thenycmaid.com/team</a>
        </div>
        <input type="text" placeholder="Search client, cleaner, address..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-[#1E2A4A] mb-4 focus:outline-none focus:border-[#1E2A4A]" />
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <div>
                ...
                <select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A] text-sm">
              ...
              <div>
                ...
                <select value={filters.service_type} onChange={(e) => setFilters({ ...filters, service_type: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A] text-sm">
                  <option value="">All</option>
              ...
              <div>
                ...
                <select value={filters.cleaner_id} onChange={(e) => setFilters({ ...filters, cleaner_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A] text-sm">
                  <option value="">All</option>
              ...
              <div>
                ...
                <select value={filters.client_id} onChange={(e) => setFilters({ ...filters, client_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A] text-sm">
                  <option value="">All</option>
              ...
              <div>
                ...
                <input type="date" value={filters.date_from} onChange={(e) => setFilters({ ...filters, date_from: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A] text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">To</label>
                <input type="date" value={filters.date_to} onChange={(e) => setFilters({ ...filters, date_to: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A] text-sm" />
              </div>
            </div>
            <div className="flex justify-between items-center">
              <p className="text-sm text-gray-600">{filteredBookings.length} booking{filteredBookings.length !== 1 ? 's' : ''}</p>
              <button onClick={clearFilters} className="text-sm text-gray-500 hover:text-[#1E2A4A]">Clear Filters</button>
            </div>
          </div>
        )}
          ...
          {loading ? <p className="p-6 text-gray-500">Loading...</p> : filteredBookings.length === 0 ? <p className="p-6 text-gray-500">No bookings found.</p> : (
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left p-3 md:p-4 font-medium text-[#1E2A4A] text-sm">Date/Time</th>
                  <th className="text-left p-3 md:p-4 font-medium text-[#1E2A4A] text-sm">Client</th>
                  <th className="text-left p-3 md:p-4 font-medium text-[#1E2A4A] text-sm">Staff</th>
                  <th className="text-left p-4 font-medium text-[#1E2A4A] text-sm hidden md:table-cell">Service</th>
                  <th className="text-left p-4 font-medium text-[#1E2A4A] text-sm hidden lg:table-cell">Rate</th>
                  <th className="text-left p-3 md:p-4 font-medium text-[#1E2A4A] text-sm">Est.</th>
                  <th className="text-left p-4 font-medium text-[#1E2A4A] text-sm hidden md:table-cell">Recurring</th>
              ...
              <tbody>
                {filteredBookings.map((b) => (
                  <tr key={b.id} className={'border-b border-gray-100 hover:bg-gray-50 cursor-pointer' + (b.status === 'pending' ? ' bg-red-50' : '')} onClick={() => openEdit(b)}>
                    <td className={'p-3 md:p-4 text-sm ' + (b.status === 'pending' ? 'text-red-700' : 'text-[#1E2A4A]')}>{formatDate(b.start_time)}</td>
                    <td className={'p-3 md:p-4 font-medium text-sm ' + (b.status === 'pending' ? 'text-red-700' : 'text-[#1E2A4A]')}>{b.clients?.name || '-'}</td>
                    ...
                    <td className={'p-3 md:p-4 text-sm ' + (b.status === 'pending' ? 'text-red-700' : 'text-[#1E2A4A]')}>~${(b.price / 100).toFixed(0)}</td>
                    ...
                    <td className="p-4 hidden md:table-cell" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => openEdit(b)} className="text-[#1E2A4A] hover:underline text-sm">Edit</button>
                        <span className="text-gray-300">|</span>
                        {b.status === 'cancelled' ? (
        ...
        <SidePanel open={showModal} onClose={() => { setShowModal(false); setEditingBooking(null) }} title="Edit Booking">
            ...
            {editingBooking.check_in_time && !editingBooking.check_out_time && (
              <div className="mb-3">
                <p className="text-sm text-green-700 bg-green-50 px-3 py-2 rounded-lg mb-2">
                  Checked in: {new Date(editingBooking.check_in_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                </p>
                <button
                  type="button"
                  onClick={async () => {
            ...
            {editingBooking.check_in_time && editingBooking.check_out_time && (
              <div className="mb-3 space-y-1">
                ...
                <p className="text-sm text-green-700 bg-green-50 px-3 py-2 rounded-lg">
                  Checked out: {new Date(editingBooking.check_out_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
