    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="p-3 md:p-6">
        <div className="flex justify-between items-start mb-2">
          <div>
            <h2 className="text-2xl font-semibold text-[#1E2A4A]">Clients</h2>
            <p className="text-gray-500">{clients.length} total clients</p>
          </div>
          <button onClick={() => setShowAddModal(true)} className="px-4 py-2 bg-[#1E2A4A] text-white rounded-lg hover:bg-[#1E2A4A]/90">+ Add Client</button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-6">
          ...
          <button onClick={() => setFilter('new')} className={`p-4 rounded-lg text-left ${filter === 'new' ? 'ring-2 ring-[#1E2A4A]' : ''} bg-[#A8F0DC]/20`}>
            <p className="text-sm text-[#1E2A4A]">New</p>
            <p className="text-2xl font-bold text-[#1E2A4A]/70">{stats.new}</p>
          </button>
        ...
        <div className="flex gap-4 mb-6">
          <input type="text" placeholder="Search clients..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 px-4 py-2 border rounded-lg text-[#1E2A4A]" />
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value as typeof sortBy)} className="px-4 py-2 border rounded-lg text-[#1E2A4A]">
        ...
        {filter !== 'all' && <button onClick={() => setFilter('all')} className="mb-4 text-sm text-[#1E2A4A] hover:underline">← Show all clients</button>}
                  ...
                  filteredClients.map(client => {
                    ...
                    return (
                      <tr key={client.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => openEditModal(client)}>
                        ...
                        <td className="px-3 md:px-4 py-3">
                          <p className="font-medium text-[#1E2A4A] text-sm">{client.name}</p>
                        ...
                        <td className="px-4 py-3 hidden md:table-cell" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center gap-2">
                            <button onClick={() => openEditModal(client)} className="text-[#1E2A4A] hover:underline text-sm">Edit</button>
            ...
            <div className="mt-8">
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  <button onClick={() => setMapFilter('all')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${mapFilter === 'all' ? 'bg-[#1E2A4A] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  ...
                  <button onClick={() => setMapFilter('new')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${mapFilter === 'new' ? 'bg-[#1E2A4A] text-white' : 'bg-[#A8F0DC]/20 text-[#1E2A4A]/70 hover:bg-[#A8F0DC]/20'}`}>
          ...
          <SidePanel open={showAddModal} onClose={closeModal} title={editingClient ? 'Edit Client' : 'Add New Client'}>
              {editingClient && (() => {
                ...
                return s ? (
                  <div className="flex gap-3 mb-4">
                    ...
                    <div className="flex-1 bg-[#A8F0DC]/20 rounded-lg p-3 text-center">
                      <p className="text-xs text-gray-500">Last Booking</p>
                      <p className="text-sm font-medium text-[#1E2A4A]">{formatDate(s.lastBooking)}</p>
                    </div>
                  </div>
                ) : null
              })()}
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A]" placeholder="John Smith" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                  <div className="flex gap-2">
                    <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="flex-1 px-3 py-2 border rounded-lg text-[#1E2A4A]" placeholder="john@email.com" />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: formatPhone(e.target.value) })} className="flex-1 px-3 py-2 border rounded-lg text-[#1E2A4A]" placeholder="212-555-1234" />
                    {editingClient && form.phone && (
                      <>
                        ...
                        <a href={`sms:${form.phone}`} className="px-4 py-2 bg-[#A8F0DC]/20 text-[#1E2A4A]/70 border border-[#A8F0DC]/30 rounded-lg text-sm font-medium hover:bg-[#A8F0DC]/20 flex items-center">Text</a>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="text" value={form.unit} onChange={(e) => { setForm({ ...form, unit: e.target.value }); setAddressChanged(true) }} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A]" placeholder="Apt 4B" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Referred By</label>
                  <select value={form.referrer_id} onChange={(e) => setForm({ ...form, referrer_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A]">
                ...
                <div>
                  ...
                  <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-[#1E2A4A]" rows={3} placeholder="Any special instructions..." />
                </div>
                {editingClient && (
                  <div className="pt-2">
                    <h4 className="text-sm font-semibold text-[#1E2A4A] mb-3">SMS Transcript</h4>
                ...
                {editingClient && !editingClient.do_not_service && (
                  <a href={`/admin/bookings?new=1&client_id=${editingClient.id}`} className="block w-full text-center py-2 px-4 bg-[#1E2A4A]/5 text-[#1E2A4A] border border-gray-200 rounded-lg font-medium hover:bg-[#1E2A4A]/10">+ Book for {form.name.split(' ')[0] || 'Client'}</a>
                )}
                {editingClient && (
                  <div className="pt-4 border-t">
                    <h4 className="text-sm font-semibold text-[#1E2A4A] mb-3">Activity</h4>
                ...
                <div className="flex justify-between items-center pt-4 border-t">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button type="submit" className="px-4 py-2 bg-[#1E2A4A] text-white rounded-lg hover:bg-[#1E2A4A]/90">{editingClient ? 'Update' : 'Add Client'}</button>
                  </div>
                </div>
              </form>
          </SidePanel>
