...
const MapComponent = dynamic(() => import('@/components/DashboardMap'), { 
  ssr: false,
  loading: () => <div className="h-[500px] bg-gray-100 rounded-lg flex items-center justify-center text-gray-500">Loading map...</div>
})
...
export default function DashboardPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setLoginError(`Invalid password. ${5 - attempts - 1} attempts remaining.`)
    }
      ...
      <main className="p-3 md:p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-4 md:mb-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 -mx-3 px-3 md:mx-0 md:px-0">
            <button onClick={() => openJobsModal('Today - Scheduled', scheduledToday.jobs)} className="flex-shrink-0 w-[100px] md:w-auto md:flex-1 bg-[#A8F0DC]/20 rounded-xl p-3 shadow-sm border border-[#A8F0DC]/30 text-left hover:shadow-md transition">
            ...
            <button onClick={() => openJobsModal('This Week - Scheduled', scheduledWeek.jobs)} className="flex-shrink-0 w-[100px] md:w-auto md:flex-1 bg-[#A8F0DC]/20 rounded-xl p-3 shadow-sm border border-[#A8F0DC]/30 text-left hover:shadow-md transition">
            ...
            <button onClick={() => openJobsModal('This Month - Scheduled', scheduledMonth.jobs)} className="flex-shrink-0 w-[100px] md:w-auto md:flex-1 bg-[#A8F0DC]/20 rounded-xl p-3 shadow-sm border border-[#A8F0DC]/30 text-left hover:shadow-md transition">
            ...
            {forecast.map((m) => (
              <button key={m.name} onClick={() => openJobsModal(`${m.name} - Scheduled`, m.jobs)} className="flex-shrink-0 w-[100px] md:w-auto md:flex-1 bg-indigo-50 rounded-xl p-3 shadow-sm border border-indigo-100 text-left hover:shadow-md transition">
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 mb-3">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="flex gap-1 bg-gray-100 p-1 rounded-lg overflow-x-auto">
                <button onClick={() => setStatusFilter('all')} className={'px-2 md:px-3 py-1 rounded-md text-xs font-medium transition whitespace-nowrap ' + (statusFilter === 'all' ? 'bg-white text-[#1E2A4A] shadow-sm' : 'text-gray-600 hover:text-[#1E2A4A]')}>
                  All ({statusCounts.all})
                </button>
                <button onClick={() => setStatusFilter('scheduled')} className={'px-2 md:px-3 py-1 rounded-md text-xs font-medium transition whitespace-nowrap ' + (statusFilter === 'scheduled' ? 'bg-[#1E2A4A] text-white' : 'text-[#1E2A4A] hover:bg-[#A8F0DC]/20')}>
                  Scheduled ({statusCounts.scheduled})
                </button>
                <button onClick={() => setStatusFilter('completed')} className={'px-2 md:px-3 py-1 rounded-md text-xs font-medium transition whitespace-nowrap ' + (statusFilter === 'completed' ? 'bg-green-500 text-white' : 'text-green-600 hover:bg-green-50')}>
                  Completed ({statusCounts.completed})
                </button>
                <button onClick={() => setStatusFilter('cancelled')} className={'px-2 md:px-3 py-1 rounded-md text-xs font-medium transition whitespace-nowrap ' + (statusFilter === 'cancelled' ? 'bg-red-500 text-white' : 'text-red-600 hover:bg-red-50')}>
                  Cancelled ({statusCounts.cancelled})
                </button>
              </div>
              <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
                <button onClick={() => setMapView('today')} className={'px-2 md:px-3 py-1 rounded-md text-xs font-medium transition ' + (mapView === 'today' ? 'bg-white text-[#1E2A4A] shadow-sm' : 'text-gray-600 hover:text-[#1E2A4A]')}>Today</button>
                <button onClick={() => setMapView('week')} className={'px-2 md:px-3 py-1 rounded-md text-xs font-medium transition ' + (mapView === 'week' ? 'bg-white text-[#1E2A4A] shadow-sm' : 'text-gray-600 hover:text-[#1E2A4A]')}>Week</button>
                <button onClick={() => setMapView('month')} className={'px-2 md:px-3 py-1 rounded-md text-xs font-medium transition ' + (mapView === 'month' ? 'bg-white text-[#1E2A4A] shadow-sm' : 'text-gray-600 hover:text-[#1E2A4A]')}>Month</button>
              </div>
            </div>
          </div>
